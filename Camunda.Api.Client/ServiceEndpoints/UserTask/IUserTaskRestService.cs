...
namespace Camunda.Api.Client.ServiceEndpoints.UserTask
{
    internal interface IUserTaskRestService
    {
        ...
        Task<Dictionary<string, VariableValue>> CompleteTask(string id, [Body] CompleteTask completeTask);

        [Post("/task/{id}/resolve")]
        Task ResolveTask(string id, [Body] ResolveTask resolveTask);

        [Post("/task/{id}/submit-form")]
        Task<Dictionary<string, VariableValue>> SubmitFormTask(string id, [Body] CompleteTask completeTask);
